...
public class IndexStoredRulePersistenceService implements RulePersistenceService {
     ...
     */
    public IndexStoredRulePersistenceService(String indexName, ClusterService clusterService, Client client, FeatureType featureType, int maxRulesPerGetRequest) {
    ...
    private void createIndexIfAbsent(ActionListener<Boolean> listener) {
        try (ThreadContext.StoredContext ctx = getContext()) {
            ...
            final CreateIndexRequest createIndexRequest = new CreateIndexRequest(indexName).settings(
                indexSettings
            );
        ...
        try (ThreadContext.StoredContext context = getContext()) {
            getRuleFromIndex(ruleId, new HashMap<>(), null, new ActionListener<>() {
                ...
                public void onResponse(GetRuleResponse getRuleResponse) {
                    ...
                    Rule updatedRule = IndexStoredRuleUtils.composeUpdatedRule(getRuleResponse.getRules().get(ruleId), request, featureType);
    ...
    public void persistUpdatedRule(String ruleId, Rule updatedRule, ActionListener<UpdateRuleResponse> listener) {
        try (ThreadContext.StoredContext context = getContext()) {
            ...
            logger.error("Error updating rule in index: {}", indexName, e);
